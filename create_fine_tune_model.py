...
def main():
    ...
    parser = argparse.ArgumentParser(description="Create a fine-tuned model using OpenAI API")
    parser.add_argument("config", help="Name of the config file (without .json extension)")
    ...
    if not os.path.exists(dataset_file_path):
        raise FileNotFoundError(f"The dataset file '{dataset_file_path}' does not exist.")
    ...
    config['ft_created_at'] = datetime.fromtimestamp(fine_tune_model_events.created_at).strftime('%Y-%m-%d %H:%M:%S')
    if fine_tune_model_events.estimated_finish:
        config['ft_estimated_finish'] = datetime.fromtimestamp(fine_tune_model_events.estimated_finish).strftime('%Y-%m-%d %H:%M:%S')
